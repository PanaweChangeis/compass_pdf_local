...
# X-Ray correlation helper
def get_xray_trace_id():
    """Get the current X-Ray trace ID from environment variables."""
    try:
        trace_header = os.environ.get('_X_AMZN_TRACE_ID', '')
        if trace_header and 'Root=' in trace_header:
            # Extract trace ID from header like: Root=1-5e1b4151-5ac6c58bb8b87dc1bd12345;Parent=53995c3f42cd8ad8;Sampled=1
            parts = trace_header.split(';')
            for part in parts:
                if part.startswith('Root='):
                    return part[5:]  # Remove 'Root=' prefix
        return None
    except Exception:
        return None

def get_correlation_context():
    """Get correlation context from X-Ray SDK if available."""
    try:
        # Try to import X-Ray SDK
        from aws_xray_sdk.core import xray_recorder
        segment = xray_recorder.current_segment()
        if segment:
            # Get annotations that contain correlation info
            annotations = getattr(segment, 'annotations', {})
            return {
                'trace_id': get_xray_trace_id(),
                'document_id': annotations.get('document_id'),
                'correlation_id': annotations.get('correlation_id'),
                'processing_stage': annotations.get('processing_stage')
            }
    except Exception:
        pass
    ...
    def put_item(
        ...
        * This function is designed to be used with this project, i.e. the DynamoDB hash 
        ...
        try:
            ...
            # Add X-Ray correlation context
            correlation_context = get_correlation_context()
            if correlation_context.get('trace_id'):
                item['xray_trace_id'] = correlation_context['trace_id']
            if correlation_context.get('processing_stage'):
                item['processing_stage'] = correlation_context['processing_stage']
