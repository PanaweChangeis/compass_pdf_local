...
class TextractParser():
    ...
    @staticmethod
    def get_textract_result_blocks(job_id: str, mode: str = 'TEXT') -> Dict[str, Any]:
        '''
        Return the textract results blocks with throttling to prevent quota violations.
        Supports both TEXT detection and document ANALYSIS modes.
        In the Textract language, a block is a `piece of text`, i.e. a sentence,
        a title, a footnote, a table, etc

        Usage
        -----
        textract_output = TextractParser.get_textract_result_blocks(job_id, mode)
        ...
        job_id
            ...
            this function.
        mode
            The mode used for the Textract job: 'TEXT' or 'ANALYZE'. Defaults to 'TEXT'.
        ...
        try:
            from helpertools.textract_throttle import create_throttled_textract_client
            textract_client, throttler = create_throttled_textract_client()
        except ImportError:
            # Fallback to regular client if throttling not available
        ...
        while True:
            logging.info(f"[TextractParser] Fetching results for job {job_id} in mode: '{mode}'")
            if mode.upper().strip() == 'ANALYZE':
                logging.info(f"[TextractParser] Using ANALYZE API for job {job_id}")
                if throttler:
                    textract_results = throttler.throttled_get_document_analysis(
                        textract_client, JobId=job_id, **extraArgs
                    )
                else:
                ...
                if throttler:
                    textract_results = throttler.throttled_get_document_text_detection(
                        textract_client, JobId=job_id, **extraArgs
                    )
                else:
                    textract_results = textract_client.get_document_text_detection(
                        JobId=job_id, **extraArgs
                    )
...
def get_textract_result_blocks(job_id: str) -> Dict:
    ...
    # check job status
    job_status = textract_client.get_document_text_detection(JobId=job_id)['JobStatus']
